///////////////////////////////////////////////////////////////////////////////
////////////////////////////// Soft Jet Tagging Infos ////////////////////////
// Author: G. Karathanasis, CERN
// Provides the input variables (features) for the SoftParT tagger. 
// Soft tagging: rely on charged CHS jets
...
class SoftParticleTransformerAK4TagInfoProducer : public edm::stream::EDProducer<> {
public:
  explicit SoftParticleTransformerAK4TagInfoProducer(const edm::ParameterSet&);
  ~SoftParticleTransformerAK4TagInfoProducer() override = default;

  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);

private:
  typedef std::vector<reco::SoftParticleTransformerAK4TagInfo> SoftParticleTransformerAK4TagInfoCollection;
...
SoftParticleTransformerAK4TagInfoProducer::SoftParticleTransformerAK4TagInfoProducer(
  ...
  produces<SoftParticleTransformerAK4TagInfoCollection>();
...
void SoftParticleTransformerAK4TagInfoProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("pfSoftParticleTransformerAK4TagInfos", desc);
}

void SoftParticleTransformerAK4TagInfoProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
 // std::cout<<" here "<<std::endl;
  auto output_tag_infos = std::make_unique<SoftParticleTransformerAK4TagInfoCollection>();
  ...
  for (std::size_t jet_n = 0; jet_n < jets->size(); jet_n++) {
     ...
     /// skip jets that fail pT or eta cuts
    ...
    // std::cout<<" flag1 "<<std::endl;
     ...
     std::vector<btagbtvdeep::SortingClass<size_t>> c_sorted, lt_sorted;
     ...
     static constexpr size_t max_pair_ = 300;
...
//           std::cout<<" flag3 "<<std::endl;
     ...
     // std::cout<<" flag4 "<<std::endl;
      int n_pair = std::min((c_sorted.size()*c_sorted.size()),max_pair_);
      features.pair_features.clear();
      features.pair_features.resize(n_pair);

      int counter=0;
      for (unsigned int itrk1 = 1; itrk1 <c_sorted.size(); itrk1++) { 
          int index1 = c_sorted.at(itrk1).get();
          auto cand1 = dynamic_cast<const reco::Candidate*>(unsubJet.daughter(index1));
          auto packed_cand1 = dynamic_cast<const pat::PackedCandidate*>(cand1);
          if (!packed_cand1) 
             continue;
          if (!packed_cand1->hasTrackDetails())
             continue;
          const reco::Track& PseudoTrack1 = packed_cand1->pseudoTrack();
          reco::TransientTrack transientTrack1;
          transientTrack1 = track_builder->build(PseudoTrack1);
          for (unsigned int itrk2 = 0; itrk2 < itrk1; itrk2++) {
              int index2 = c_sorted.at(itrk2).get();
              auto cand2 = dynamic_cast<const reco::Candidate*>(unsubJet.daughter(index2));
              auto packed_cand2 = dynamic_cast<const pat::PackedCandidate*>(cand2);
              if (!packed_cand2)
                 continue;
              if (!packed_cand2->hasTrackDetails())
                 continue;
              const reco::Track& PseudoTrack2 = packed_cand2->pseudoTrack();
              reco::TransientTrack transientTrack2;
              transientTrack2 = track_builder->build(PseudoTrack2);
              auto& pair_features = features.pair_features.at(counter);
              btagbtvdeep::trackPairsToFeatures(transientTrack1,transientTrack2,jet,pv,pair_features);
              if (counter+1==n_pair) break;
              counter++;
         }
      }
      features.pair_features.resize(counter);
...
DEFINE_FWK_MODULE(SoftParticleTransformerAK4TagInfoProducer);
